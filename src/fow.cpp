...
void FoW::updateTile(int x, int y) {
	const neighbour_t* neiba = neighbours; // for shortness
	if (!contains(x, y) || status[y][x] == REVEALED)
		return;

	int index = 0;
	for (int i = neighbours_len - 1; i >= 0; i--) {
		int bit;
		if (contains(x + neiba[i].j, y + neiba[i].i))
			bit = (status[y + neiba[i].i][x + neiba[i].j] == REVEALED);
		else
			bit = 0;
		index = index * 2 + bit;
	}
